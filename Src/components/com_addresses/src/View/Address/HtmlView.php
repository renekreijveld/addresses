...
namespace Joomla\Component\Addresses\Site\View\Address;
...
class HtmlView extends BaseHtmlView
{
    ...
    private function hasAccess(): bool
    {
        ...
        if ($this->_layout == 'edit') {
            ...
            if ($isEdit) {
                $authorised = $user->authorise('core.edit', 'com_addresses');
                $access     = new AccessHelper();
                $access->preloadOwnRecords('#__addresses');
                if ($access->canAccessOwnRecord()) {
                    return true;
                }
            } else {
                $authorised = $user->authorise('core.create', 'com_addresses');
            }
            if ($authorised !== true) {
    ...
    protected function setupDocument(): void
    {
        ...
        if ($menu) {
            ...
            $this->params->def('page_heading', Text::_('COM_ADDRESSES_DEFAULT_PAGE_TITLE'));
        }
